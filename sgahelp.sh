...
function service_status() {
    ...
    if echo "$state" | grep active >/dev/null 2>&1; then
        echo "RUNNING"
    else
        echo "DOWN"
    fi
...
function main_menu() {
    ...
    echo "9. Enable SSH password authentication"
    echo "10. Exit"
    echo " "
    ...
    2)
        ...
        #       disable_service app_mgmt_web_secure.service      # suspicion that FRP8 will fail if stopped
        read -p "Press enter to go back on main menu"
        main_menu
        ;;
    3)
        ...
        echo "FRP8 (UDP only) connections:"
        FRP8
        echo "FRP7 connections:"
        FRP7
        read -p "Press enter to go back on main menu"
        main_menu
        ;;
